...
function Home({ auth: { user }, profile: { profile } }) {
  ...
  return (
    <Container>
      ...
      {usersSavedBooks.favorites.length === 0 &&
      usersSavedBooks.readLater.length === 0 ? (
        <Container style={{ textAlign: "center", marginTop: "100px" }}>
          <CloudPlus size={100} />
          <h3>Start saving books to your personal base!</h3>
          <Button style={{ marginTop: "20px" }} href="/send-new-reco/books">
            Search for books
          </Button>
        </Container>
      ) : null}
      ...
      {usersFriends.length === 0 ? (
        <Container style={{ textAlign: "center", marginTop: "100px" }}>
          <PersonPlus size={100} />
          <h3>
            If you have a friend using our platform please search for their
            username in the search bar on your top right corner. (Search is case
            sensitive!)
          </h3>
        </Container>
      ) : null}
