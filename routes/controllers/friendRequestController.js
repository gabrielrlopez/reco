...
exports.checkIfAlreadyRequestedOrFriends = catchErrorsAsync(
  async (req, res, next) => {
    ...
    if (friends.includes(receiverUserId))
      next(new AppError("This user is already your friend."));
    if (sentRequest.includes(receiverUserId))
      next(new AppError("You already sent this user a friend request."));
    if (requests.includes(receiverUserId))
      next(new AppError("This user has already sent you a friend request."));
      ...
      $push: {
        friends: {
          $each: [
            {
              userFullName: [
                senderUserProfile.firstName,
                senderUserProfile.lastName,
              ],
              userId: senderUserProfile._id,
              userName: senderUserProfile.userName,
              photo: senderUserProfile.photo,
            },
          ],
          $sort: { userFullName: 1 },
        },
